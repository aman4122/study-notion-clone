...
exports.updateSectionOrder = async (req, res) => {
    try {
        ...
        if (!courseId || !sectionOrder || !Array.isArray(sectionOrder)) {
            return res.status(400).json({
                success: false,
                message: "Missing required properties or invalid format"
            });
        }

        const updatedCourse = await Course.findByIdAndUpdate(
            courseId,
            { courseContent: sectionOrder },
            { new: true }
        ).populate({
            path: "courseContent",
            populate: {
        ...
        return res.status(500).json({
            success: false,
            message: "Could not update section order",
            error: err.message
        });
    }
}